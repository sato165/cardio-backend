...
class PredictionInput(BaseModel):
    """
    Variables de entrada para el modelo final (notebook_proy_final).
    Columnas alineadas con columnas_modelo.pkl — k=4 clusters.
    talla se recibe en cm y se convierte a metros en preprocessing.py
    """

    # Laboratorio
    c_total:        float = Field(..., ge=0,  description="Colesterol total (mg/dL)")
    ...
    talla:          float = Field(..., ge=127.0, le=197.0,  description="Talla en cm (se convierte a metros internamente)")
    ...
    # --- Opcionales para Framingham / SCC ---
    colesterol_total_mgdl:        Optional[float] = Field(None, ge=50,  le=500, description="Colesterol total (mg/dL) – Framingham")
    diabetes:                     Optional[int]   = Field(None, ge=0,   le=1,   description="0 no diabético, 1 diabético – Framingham")
    tratamiento_antihipertensivo: Optional[int]   = Field(None, ge=0,   le=1,   description="¿Recibe tratamiento antihipertensivo?")
    fuma:                         Optional[int]   = Field(None, ge=0,   le=1,   description="¿Fuma actualmente?")
    ...
    def validar_presion(self) -> "PredictionInput":
        ...
        if self.ta_diastolica >= self.ta_sistolica:
            raise ValueError(
                f"Presión diastólica ({self.ta_diastolica}) debe ser menor "
                f"que la sistólica ({self.ta_sistolica})"
            )
        return self